...
function isNewInTwoMonths(dateStr) {
  let dateParts = dateStr.split("-");
  let dateObj = new Date(dateParts[0], dateParts[1] - 1, dateParts[2]);
  let currentDate = new Date();
  let twoMonthsAgo = new Date(currentDate.setMonth(currentDate.getMonth() - 2));
...
  // if item.date is not out of 2 monthes, add "New" Tag to abstract
  let mappedProductArr = arr.map(function (item) {
    if (isNewInTwoMonths(item.date)) {
      // abstract not empty
      if (item.abstract.trim() != "") {
        // if no "New", add "New" to abstract
        if (item.abstract.indexOf("New") < 0) {
          item.abstract = item.abstract + ",New";
        }
      } else {
        // if abstract is empty, add "New"
        item.abstract = item.abstract + "New";
      }
    }
    return item;
...
  //product has 'new' tag, pin to top in the list
  return mappedProductArr.sort(function (a, b) {
